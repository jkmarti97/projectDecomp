...
## Scope Analysis
### Count & Classification Level: 1 — Portfolio Epic
### Scale/Timebox
- Rule applied: Portfolio Epic = strategic initiative that spans multiple Agile Release Trains (ARTs) or value streams, requires significant cross-business investment/coordination, or is expected to span more than 6 months. If scope cannot reasonably be delivered by one ART within a single PI (~8–12 weeks) it is Portfolio Epic-level.
- Rationale (1–2 sentences): The brief describes a cross-organizational program (detection engineering, telemetry/platform, Snowflake/data, and security operations) to implement detection-as-code, telemetry normalization/ingestion, Snowflake pipelines, governance and enablement. Because this work requires coordination across multiple teams/value streams and sustained investment (per the Portfolio Epic heuristic), it should be scoped as 1 Portfolio Epic.
### Scoped Deliverable(s) (count = 1)
- Title: Detection Engineering Automation with Anvilogic
  - Summary (2–4 sentences): Deliver a cross-organization capability to automate detection engineering using Anvilogic that includes: mapping existing detections to MITRE ATT&CK; prioritized telemetry normalization and ingestion into a governed data platform (Snowflake); and a detection-as-code lifecycle with versioning, auditability, and enablement (processes/training). MVP (minimal releasable increment): an end-to-end pilot that ingests one prioritized telemetry source into Snowflake, deploys a detection-as-code repo with CI/CD for one representative detection, and has stakeholder-accepted governance/checklist for handover. Out of scope: upstream rearchitecture of unrelated telemetry producers and broad enterprise Snowflake migration; numeric success targets in the brief are treated as proposed and must be validated with stakeholders.
